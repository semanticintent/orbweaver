...
export class SvgRenderer implements Renderer<string> {
  render(scene: Scene, options: SvgRenderOptions = {}): string {
    ...
    const sceneMarkup = `<g class="ow-scene"${headerHeight === 0 ? '' : ` transform="translate(0 ${headerHeight})"`}><rect class="ow-canvas" width="${scene.width}" height="${scene.height}" rx="12"/><g class="ow-groups">${groupSurfaces}</g><g class="ow-edges" role="list" aria-label="Relationships">${edges}</g><g class="ow-group-labels">${groupLabels}</g><g class="ow-nodes" role="list" aria-label="Entities">${nodes}</g></g>`
