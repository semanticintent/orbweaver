...
import { darkTheme, layoutGraph, lightTheme, renderHtmlArtifact, renderSvgArtifact, summarizeGraph } from '../dist/index.js'
...
for (const showcase of showcases) {
  ...
  const exportProvenance = { renderer: `Orbweaver ${packageVersion}`, source: { file: 'examples/showcases.mjs', path: showcase.slug } }
  const lightArtifact = await renderSvgArtifact(showcase.graph, { layout: showcase.layout, render: { theme: lightTheme, frame }, provenance: exportProvenance })
  const darkArtifact = await renderSvgArtifact(showcase.graph, { layout: showcase.layout, render: { theme: darkTheme, frame }, provenance: exportProvenance })
  const light = lightArtifact.data
  const dark = darkArtifact.data
  const darkMarkup = dark.replace(/^<\?xml[^>]+>\s*/, '')
  ...
  rendered.push({ ...showcase, scene, light, dark, darkMarkup, summary: summarizeGraph(scene.graph) })
}
...
const cards = rendered.map((showcase, index) => {
  ...
  return `<article class="showcase" id="${showcase.slug}">
    <header class="showcase-header">
      ...
      <div class="actions"><a href="${showcase.slug}.html">Portable HTML</a><button type="button" data-png="${showcase.slug}">PNG · 2×</button><a href="${showcase.slug}-dark.svg" download>Dark SVG</a><a href="${showcase.slug}-light.svg" download>Light SVG</a></div>
    </header>
    <div class="diagram" data-showcase="${showcase.slug}">${showcase.darkMarkup}</div>
